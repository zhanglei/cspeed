...
reset_model_sql(zval *this) /*{{{ To clear the current sql result for the next execute */
{
    zend_update_property_string(
        cspeed_model_ce, 
        this, 
        CSPEED_STRL(CSPEED_MODEL_WHERE_COND),
        ""
    );
    zend_update_property_string(
        cspeed_model_ce, 
        this, 
        CSPEED_STRL(CSPEED_MODEL_ORDER_BY), 
        ""
    );
    zend_update_property_string(
        cspeed_model_ce, 
        this, 
        CSPEED_STRL(CSPEED_MODEL_SELECT), 
        "*"
    );
    zend_update_property_string(
        cspeed_model_ce, 
        this, 
        CSPEED_STRL(CSPEED_MODEL_GROUP_BY), 
        ""
    );
}/*}}}*/
...
get_fields_from_object(zval *object, zval *ret_val) /*{{{ Get the fields from the object */
{
    ...
    zval *columns = zend_read_property(
        cspeed_model_ce, 
        object, 
        CSPEED_STRL(CSPEED_MODEL_COLUMNS), 
        1, 
        NULL
    );
    if (columns == NULL) return ;

    zval *new_record  = zend_read_property(
        cspeed_model_ce, 
        object, 
        CSPEED_STRL(CSPEED_MODEL_NEW_RECORD), 
        1, 
        NULL
    );
    zval *primary_key = zend_read_property(
        cspeed_model_ce, 
        object, 
        CSPEED_STRL(CSPEED_MODEL_PRIMARY), 
        1, 
        NULL
    );
    ...
    zval *array_data  = zend_read_property(
        cspeed_model_ce, 
        object, 
...
set_columns_to_model(zval *model_object)
{
    ...
    zval *table_name = zend_read_property(
        cspeed_model_ce, 
        model_object, 
        CSPEED_STRL(CSPEED_MODEL_TABLE_NAME), 
        1, 
        NULL
    );
    zval *pdo_object_set = zend_read_property(
        cspeed_model_ce, 
        model_object, 
        CSPEED_STRL(CSPEED_MODEL_PDO_OBJECT), 
        1, 
        NULL
    );
    zend_string *columns_sql = strpprintf(
        0, 
        "SHOW COLUMNS FROM `%s`", 
    ...
    if ( !output_sql_errors(&pdo_statement, model_object) ){
        ...
        ZEND_HASH_FOREACH_VAL(Z_ARRVAL(result), each_column) {
            if ( each_column && ( Z_TYPE_P( each_column ) == IS_ARRAY) ) {
                zval *column_name = zend_hash_find(
                    Z_ARRVAL_P(each_column), 
                    strpprintf(
                        0, "Field"
                    )
                );
                if ( column_name != NULL ) {
                    add_next_index_string(
                        &column_data, 
                        Z_STRVAL_P(column_name)
                    );
                }
                if ( !is_set_primary ) {
                    /* The primary key name, if set */
                    zval *primary_key = zend_hash_find(
                        Z_ARRVAL_P(each_column), 
                        strpprintf(
                            0, 
                            "Key"
...
initialise_the_model_object(zval *model_object, zend_long new_record, INTERNAL_FUNCTION_PARAMETERS)/*{{{*/
{
    ...
    zval *pdo_object = zend_read_static_property(
        cspeed_adapter_ce, 
    ...
    zval *is_set_pdo = zend_read_property(
        cspeed_model_ce, 
        model_object, 
        CSPEED_STRL(CSPEED_MODEL_PDO_OBJECT), 
        1, 
    ...
    zend_update_property_long(
        cspeed_model_ce, 
        model_object, 
        CSPEED_STRL(CSPEED_MODEL_AS_ARRAY), 
        IS_FALSE
    );
    ...
    if ( !ZVAL_IS_NULL(&magic_datas) && (Z_TYPE_P(&magic_datas) == IS_ARRAY)  
        ...
        ZEND_HASH_FOREACH_STR_KEY_VAL(Z_ARRVAL_P(&magic_datas), var_key, var_value) {
            ...
            smart_str_appendc(
                &fields_str, 
                ','
            );                         /* After finished, the value is (id, i, t, */
            /* VALUES */
        ...
        *(ZSTR_VAL(datas_str.s) + ZSTR_LEN(datas_str.s) - 1) = ')';
        ...
        zend_string *result = strpprintf(
            0, 
            "%s%s", 
            ZSTR_VAL(fields_str.s), 
            ZSTR_VAL(datas_str.s)
        );
...
{
    zval *is_set_ultimate_pdo = zend_read_property(
        cspeed_model_ce, 
        this, 
        CSPEED_STRL(CSPEED_MODEL_ULTIMATE_PDO), 
        1, 
        NULL
    );
...
set_executed_sql(zval *model_object, zend_string *sql) /*{{{ Set the executed sql to the model */
{
    zend_update_property_str(
        cspeed_model_ce, 
        model_object, 
        CSPEED_STRL(CSPEED_MODEL_EXECUTED_SQL), 
        sql
    );
}/*}}}*/
...
CSPEED_METHOD(Model, one)/*{{{ proto Model::one()*/
{
    ...
    zval *as_array   = zend_read_property(
        cspeed_model_ce, 
        getThis(), 
        CSPEED_STRL(CSPEED_MODEL_AS_ARRAY),   
        1, 
...
CSPEED_METHOD(Model, save)/*{{{ proto Model::save()*/
{
    ...
    if (Z_LVAL_P(new_record) == IS_TRUE) {
        /* INSERT INTO */
        execute_sql = strpprintf(
            0, 
            "INSERT INTO %s %s",  /* INSERT INTO xx (id, t, r) VALUES(33, 8, 99) */
            Z_STRVAL_P(table_name),
        ...
        execute_sql = strpprintf(
            0, 
            "UPDATE %s SET %s%s", /* UPDATE xx SET a=3,b=3,d=2 */
            Z_STRVAL_P(table_name),
...
CSPEED_METHOD(Model, delete)/*{{{ proto Model::delete()*/
{
    ...
    zend_string *execute_sql = strpprintf(
        0, 
        "DELETE FROM %s%s", 
        Z_STRVAL_P(table_name), 
        Z_STRVAL_P(where)
    ...
    if ( CSPEED_G(di_object) ){
        ...
        if (EXPECTED( (db_adapter = zend_hash_find(Z_ARRVAL_P(objects), db_name) ) != NULL )) 
        {
            ...
            zend_update_property(
                cspeed_model_ce, 
                getThis(), 
                CSPEED_STRL(CSPEED_MODEL_ADAPTER), 
                db_adapter
            );
...
CSPEED_METHOD(Model, getDb)/*{{{ Get the PDO object*/
{
    ...
    is_set_ultimate_pdo = zend_read_property(
        cspeed_model_ce, 
        getThis(), 
        CSPEED_STRL(CSPEED_MODEL_PDO_OBJECT), 
        1, 
...
{
    zval *adapter_object = zend_read_property(
        cspeed_model_ce, 
        getThis(), 
        CSPEED_STRL(CSPEED_MODEL_ADAPTER), 
        1, 
    ...
    adapter_object = zend_read_static_property(
        cspeed_adapter_ce, 
...
{
    zval *executed_sql = zend_read_property(
        cspeed_model_ce, 
        getThis(), 
        CSPEED_STRL(CSPEED_MODEL_EXECUTED_SQL), 
        1, 
...
CSPEED_INIT(model)  /*{{{ Load the module function*/
{
    ...
    zend_declare_property_null(
        cspeed_model_ce, 
        CSPEED_STRL(CSPEED_MODEL_MAGIC_DATAS), 
        ZEND_ACC_PRIVATE
    );
    zend_declare_property_null(
        cspeed_model_ce, 
    ...
    zend_declare_property_null(
        cspeed_model_ce, 
        CSPEED_STRL(CSPEED_MODEL_RESULT), 
        ZEND_ACC_PRIVATE
    );
    zend_declare_property_null(
        cspeed_model_ce, 
    ...
    zend_declare_class_constant_string(
        cspeed_model_ce, 
        CSPEED_STRL(EVENT_BEFORE_SAVE), 
        EVENT_BEFORE_SAVE
    );
    zend_declare_class_constant_string(
        cspeed_model_ce, 
        CSPEED_STRL(EVENT_AFTER_SAVE), 
        EVENT_AFTER_SAVE
    );
    zend_declare_class_constant_string(
        cspeed_model_ce, 
        CSPEED_STRL(EVENT_BEFORE_DELETE), 
        EVENT_BEFORE_DELETE
    );
    zend_declare_class_constant_string(
        cspeed_model_ce, 
        CSPEED_STRL(EVENT_AFTER_DELETE), 
        EVENT_AFTER_DELETE
    );
}/*}}}*/
