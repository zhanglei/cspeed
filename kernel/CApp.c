...
int cspeed_deal_reqeust(zend_string *url, zend_fcall_info *zfi, zend_fcall_info_cache *zfic, zval *ret_val)
/*{{{ Deal the REQUEST */
{
    zval zval_path_info;
    ...
    if (CSPEED_STRING_NOT_EMPTY(path_info) || ((*(ZSTR_VAL(url)) == '/') && (ZSTR_LEN(url) == 1)))
    {
...
CSPEED_METHOD(App, autoload) /*{{{ proto App::autoload The cspeed framework's autoload system */
{
    ...
    if (zend_parse_parameters(
            ZEND_NUM_ARGS() TSRMLS_CC,
            "S",
            &class_name_with_namespace) == FAILURE)
    {
        return;
    }
    /* To load the file */
