# CSpeed PHP Micro Framework v1.3.8 #
...
1、修复路由规则中模块初始化路由的无效问题

	v1.3.1版本：在单个模块目录下初始化模块的路由后，跳转其余模块的异常BUG
	
2、不设置默认模块的情况下，系统按照两段URL进行解析。
...
2、修复BUG
	...
	１、修复在系统的View引擎部分由于变量未初始化导致的nginx异常
